...
<pre>
packer build --only=vmware-iso ztps-fedora_20_x86_64.json
</pre>
    Some logs worth noting
    <pre>
    Arista$ packer build ztps-fedora_20_x86_64.json
    ...
    After 10 minutes the OS installation will be complete, the VM will reboot, and you will be presented with a login prompt.  **Resist the urge to log in and tinker** - the [setup.sh](https://github.com/arista-eosplus/packer-ztpserver/blob/master/Fedora/scripts/setup.sh) script is about to be kicked off.
    ...
    After some extensive yumming (~5minutes), you will see:
    <pre>
...
packer build --only=virtualbox-windows-iso ztps-fedora_20_x86_64.json
    ...
    Some logs worth noting:
    <pre>
    Arista$ packer build --only=virtualbox-iso ztps-fedora_20_x86_64.json
    ...
    After a few minutes the OS installation will be complete, the VM will reboot, and you will be presented with a login prompt. **Resist the urge to log in and tinker** - the [setup.sh](https://github.com/arista-eosplus/packer-ztpserver/blob/master/Fedora/scripts/setup.sh) script is about to be kicked off.
    Meanwhile, you'll notice the packer builder ```ssh``` into the VM and begin working on updating, installing and configuring new services.
    <pre>
